...
echo "    2. kubectl apply -k k8s/overlays/dev -n infra --server-side 2>/dev/null || kubectl apply -k k8s/overlays/dev"
echo "    3. Nếu vẫn lỗi (đổi password sau lần deploy đầu): bash scripts/reset-mongodb.sh"
